...
const drawerTitle = computed(() => (isEditMode.value ? '编辑流配置' : '流配置'))
...
watch(
  () => [formData.ipAddress, formData.port, formData.username, formData.password, formData.streamType],
  () => {
    formData.sourceUrl = autoSourceUrl.value
  },
  { immediate: true }
)
...
const syncGroupName = () => {
  const matched = groupOptions.value.find((item) => item.id === formData.groupId)
...
const open = async (vo: CameraDeviceVO, mode: OpenMode = 'view') => {
  ...
  if (deviceId) {
    ...
    try {
      ...
      Object.assign(formData, defaultFormData, formRes.data)
      formData.deviceId = formData.deviceId || deviceId
...
<template>
  ...
  >
    ...
    >
      <div class="mode-bar">
        <el-tag v-if="!isEditMode" type="info">当前为展示模式</el-tag>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="分组" prop="groupId">
            <el-select v-model="formData.groupId" placeholder="请选择分组" style="width: 100%" @change="syncGroupName" :disabled="!isEditMode">
              <el-option
        ...
        <el-col :span="12">
          <el-form-item label="设备UUID">
            <el-input v-model="formData.deviceUuid" placeholder="设备唯一标识" readonly />
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="路径UUID">
            <el-input v-model="formData.pathUuid" placeholder="路径唯一标识" readonly />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="24">
          <el-form-item label="源流地址" prop="sourceUrl">
            <el-input
              :model-value="formData.sourceUrl || '--'"
              placeholder="按 IP/端口/账号/密码/码流类型自动生成"
              readonly
            />
          </el-form-item>
        </el-col>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="24">
          <el-form-item label="录像格式" prop="recordFormat">
            <el-input v-model="formData.recordFormat" placeholder="例如：fmp4" :disabled="!isEditMode" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button v-if="isEditMode" type="primary" :loading="loading" @click="onSave">
          保存
        </el-button>
      </span>
    </template>
